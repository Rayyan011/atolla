    ...
    <section className="relative w-full min-h-screen overflow-hidden flex items-center justify-center pt-16">
      ...
      <div className="relative z-10 max-w-7xl w-full mx-auto px-6 md:px-12">
        <div className="max-w-3xl animate-float">
          <h1 className="font-playfair text-4xl md:text-6xl lg:text-7xl font-bold text-white mb-4 text-glow leading-tight">
            Illuminate the <span className="text-atolla-purple">Unknown</span>
          </h1>
          <h2 className="font-cormorant text-2xl md:text-3xl text-white/90 coral-glow mb-8">
            Regenerate the <span className="text-atolla-coral">Future</span>
          </h2>
          <div className="space-y-6">
            <p className="text-white/80 text-lg md:text-xl max-w-2xl border-l-4 border-atolla-purple pl-4">
              Where science meets storytelling, data transforms into action, and ocean mysteries become visible.
            </p>
            <Button 
              className="bg-gradient-to-r from-atolla-coral to-atolla-purple hover:from-atolla-purple hover:to-atolla-coral text-white rounded-full px-8 py-6 text-lg transition-all hover:shadow-lg hover:shadow-atolla-coral/20"
            >
              Swim Closer
            </Button>
          </div>
      ...
      {/* Wave overlay */}
      <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-atolla-sand to-transparent z-10"></div>
    </section>
  );
};
